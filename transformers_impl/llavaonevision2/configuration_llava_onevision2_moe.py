...
"""LlavaOnevision2 MoE model configuration (transformers 5.x aligned).

Mirrors the structure of `configuration_llava_onevision2.py` (dense) which is
already bit-exact aligned with the deployed remote_code in
`ax_stage2_0_instruct_p_26_04_17_ablation_codec_img_sft_iter3000_hf`.

MoE-specific deltas vs. the dense config:
  * `model_type = "llava_onevision2_moe"`
  * `text_config` defaults to `qwen3_moe` (resolved via `CONFIG_MAPPING`)
  * Adds `output_router_logits` / `router_aux_loss_coef` for the MoE
...
class LlavaOnevision2MoeConfig(PreTrainedConfig):
    ...
    model_type = "llava_onevision2_moe"
    # `text_config` is resolved dynamically based on its `model_type` (defaults to `qwen3_moe`),
    ...
    def __post_init__(self, **kwargs):
        ...
        # Resolve text_config dynamically via CONFIG_MAPPING (defaults to qwen3_moe)
        if isinstance(self.text_config, dict):
            text_model_type = self.text_config.get("model_type", "qwen3_moe")
            ...
            text_config_cls = CONFIG_MAPPING["qwen3_moe"]
            self.sub_configs["text_config"] = text_config_cls
